...
func TestSpentTimeLockP2wsh(t *testing.T) {
	...
	tx.LockTime = uint32(lockTime.Unix()) // 设置交易的 LockTime 为时间锁的时间戳

	// 添加输入 - 时间锁UTXO
	require.NoError(t, err)
	outPoint := wire.NewOutPoint(prevHash, 0)
	txIn := wire.NewTxIn(outPoint, nil, nil)
	txIn.Sequence = 0xFFFFFFF0 // 使用 0xFFFFFFFE 作为时间锁交易的序列号
	tx.AddTxIn(txIn)
	...
	// 计算交易费用
	fee := int64(1000) // 1000 satoshis作为固定费用
